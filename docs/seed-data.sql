...
FROM (VALUES
    ...
    ('Dealer', 'Unassigned mirrored forms', NULL),
    ('Dealer', 'Dealer Manufacturing', 'SR2 5CC'),
    ('Dealer', 'Dealer Logistics', 'SR2 6DD')
...
FROM (VALUES
    ...
    ('Dealer', 'Dealer Manufacturing', 'Sophie', 'Turner', 'sophie.turner@dealer.example', 'SR2 5CC'),
    ('Dealer', 'Dealer Manufacturing', 'Leo', 'Watson', 'leo.watson@dealer.example', 'SR2 5CC'),
    ('Dealer', 'Dealer Logistics', 'Grace', 'Mitchell', 'grace.mitchell@dealer.example', 'SR2 6DD'),
    ('Dealer', 'Dealer Logistics', 'Noah', 'Foster', 'noah.foster@dealer.example', 'SR2 6DD')
...
FROM (VALUES
    ...
    ('Dealer', 'Sales form', 'Dealer Manufacturing', NULL, 'Initial sales qualification', 'Pipeline stage: Proposal', CAST('2026-03-02T10:15:00' AS datetime2(7)), CAST('2026-03-09T15:45:00' AS datetime2(7))),
    ('Dealer', 'Sales form', NULL, 'noah.foster@dealer.example', 'Follow-up call outcome', 'Requested pricing review', CAST('2026-03-04T13:00:00' AS datetime2(7)), CAST('2026-03-11T09:30:00' AS datetime2(7)))
...
FROM (VALUES
    ...
    ('Dealer', 'sophie.turner@dealer.example', 'Waiting for pricing confirmation from procurement.'),
    ('Dealer', 'noah.foster@dealer.example', 'Confirmed follow-up workshop for next week.')
